...
class RestAPIMovie: ViewModel() {
    companion object {
        private const val API_KEY = "1c82c78abfacc7ee0508966c9489f84c"
    }
    private val listMovie = MutableLiveData<ArrayList<ParcelableData>>()

    internal fun CallData(context: Context,search:String) {
        val queue = Volley.newRequestQueue(context)
        val url: String
        if(search=="") url = "https://api.themoviedb.org/3/discover/movie?api_key=$API_KEY&language="+ context.getString(R.string.language)
        else url = "https://api.themoviedb.org/3/search/movie?api_key=$API_KEY&language="+context.getString(R.string.language)+"&query="+search
